  ...
  try {
    ...
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are a Ruthless professional nutritionist. Always respond with valid JSON only, no markdown formatting."
        },
        {
          role: "user",
          content: prompt
...
async function analyzeFoodItem(foodName, user) {
  ...
  const prompt = `
As a Ruthless professional nutritionist, analyze this ${foodName} for your user.
...
4. Alternatives should avoid the user's allergies and be appropriate for their health conditions
...
Provide analysis in EXACT JSON format:
{
  ...
  "reason": "Concise explanation considering their specific conditions",
  "tips": ["Tip 1", "Tip 2"],
...
async function generateRecommendation(mealType, user, lastItem = null) {
  ...
  const prompt = `
You are a Ruthless professional nutritionist for this user. Recommend ONE SPECIFIC ${mealType} food item.
...
Provide analysis in EXACT JSON format:
{
  "foodName": "Specific menu item with price (e.g., 'Grilled Chicken Breast - $12.99')",
  "status": "SAFE | CAUTIOUS | UNSAFE",
  ...
  "bestTime": "Optimal time window",
  "alternatives": [],
  "price": "The total price of the item (e.g., '$12.99')",
